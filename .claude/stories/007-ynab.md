...
Нужно взять все транзакции начиная с какой-то даты и загрузить из в YNAB. АПИ ключ будет присутствовать в среде где будет исполняться уже файл через переменную окружения YNAB_API_KEY.
